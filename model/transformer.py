...
class Embeddings(nn.Module):
		...
		def __init__(self, d_model=512, num_classes=10, d_cls=100, d_noise=100):
				super(Embeddings, self).__init__()
				self.class_embedding = nn.Embedding(num_classes, d_cls)
				self.d_model = d_model
				self.embedding = nn.Linear(d_cls+d_noise+4, d_model)

		''' !!!这个地方还需要重写 '''
		def forward(self, classes, bbox, noise):
				'''
						Params:
								classes: 物体类别
								bb: bounding box信息
				...
				output = self.embedding(torch.cat((bbox, cls_emb, noise), dim=-1))
...
class PositionalEncoding(nn.Module):
		...
		def forward(self, boxes):
				...
				x, y = (x0 + x1) / 2, (y0 + y1) / 2
				pos = (x * self.W + y).unsqueeze(2)
				# print(pos.size())
				pe_fe = torch.arange(self.d_model).view(1, 1, -1).repeat(x.shape[0], x.shape[1], 1).to(x.device)
...
class TransformerEncoder(nn.Module):
		...
		def __init__(self,
				...
				self.encoder_layers = nn.ModuleList(
						[SingleEncoderLayer(d_model, num_heads, d_ffn, dropout) for _ in range(num_trans_layers)])

				# !!!这里需要根据Embeddings和PositionalEncoding的实现修改
				self.input_embedding = Embeddings(d_model=d_model, num_classes=num_classes, d_noise=d_noise)
				self.pos_encoding = PositionalEncoding(d_model)

		def forward(self, object_classes, boxes, noise):
				# !!!这里需要根据Embeddings和PositionalEncoding的实现修改
				output = self.input_embedding(object_classes, boxes, noise)
				# output = torch.FloatTensor(4, 6, 128).to(object_classes.device)
