...
class CIPSskip(nn.Module):
	...
	def forward(self,
		...
		'''
			v_map: bod->bd (option: reshape+affine/mean) (here mean value temp currently)
			latent: bd
		...
		if not input_is_latent:
			...
			latent = self.coaffine(torch.cat((v_map.mean(dim=1), latent), dim=-1)).view(batch_size, -1)


		x = mask_feat.mean(dim=1)
...
class CIPSskipObj(nn.Module):
	...
	def forward(self,
		...
		if not input_is_latent:
			latent = self.style(latent)  ## generate W
			latent = self.coaffine(torch.cat((v_map, latent), dim=-1)).view(batch_size, -1)
