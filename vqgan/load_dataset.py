...
class VgSceneGraphDataset(Dataset):
    def __init__(self, vocab, h5_path, image_dir, image_size=(256, 256),
        ...
        """
        :param vocab: 存储了所有的类别与其对应的序号
        :param h5_path: h5文件位置
        :param image_dir: 图像数据所在位置
        :param image_size: Resize的图像尺寸
        :param normalize_image: 是否要对图像数据归一化
        :param max_objects:
        :param max_samples:
        :param include_relationships:
        :param use_orphaned_objects:
        :param left_right_flip: 是否左右翻转
        """
...
def get_VgSceneGraphDataset():
    """
    获取VgSceneGraph数据集
    :return dataset: 数据集
    :return vocab: 词典
    :return num_classes: 物体类别总数
    """
    h5_path = '/home/pc/Mine/project/GitRepository/LayoutToImage/transLayout/data/train.h5'
    import json
    with open('/home/pc/Mine/project/GitRepository/LayoutToImage/transLayout/data/vocab.json', 'r') as f:
        vocab = json.load(f)
        num_classes = len(vocab['object_name_to_idx'])
    image_dir = '/home/pc/Mine/project/GitRepository/LayoutToImage/transLayout/data/images/'
    dataset = VgSceneGraphDataset(vocab, h5_path, image_dir, image_size=(256, 256),
                                  normalize_images=True, max_objects=10, max_samples=None,
                                  include_relationships=True, use_orphaned_objects=True,
                                  left_right_flip=False)
    return dataset, vocab, num_classes
...
if __name__ == '__main__':
    ...
    #                              include_relationships=True, use_orphaned_objects=True,
    #                              left_right_flip=False)
    dataset, vocab, num_classes = get_VgSceneGraphDataset()
    ...
    for batch in loader:
        print(batch)
        break
    # real_images, objects, boxes = dataset.__getitem__(0)
    # real_images, objects, boxes = next(loader)
    #print(real_images)
    # from utils.util import extract_obj_layer
    # extract_obj_layer(real_images, boxes, objects, 64)
